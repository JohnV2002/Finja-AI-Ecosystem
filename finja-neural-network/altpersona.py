...
=======================
AltPersona: YourAI's rebellious sister. 😈

- altpersona_brat_node: when Granite blocks and no password is present
- altpersona_uncensored_node: full context, uncensored model
...
def altpersona_brat_node(state, debug: Any = None) -> Dict[str, Any]:
    """
    AltPersona Brat Mode - minimal response when Granite blocks.
    No password = no service. Whatever.
    ...
    Returns:
        Dict[str, Any]: {"final_response": str}.
    """
    if debug is None:
        class DummyDebug:
...
def altpersona_uncensored_node(state, debug: Any = None) -> Dict[str, Any]:
    """
    AltPersona Uncensored Mode - YourAI's rebellious sister with full context! 😈

    Has access to memories, history, guest context, and tools.
    Uses the uncensored model (dolphin-llama3).

    Args:
        state: The pipeline state (question, memories, history, user_name, ...).
        debug (Any): Optional dashboard debug client.
    ...
    # ==========================================
    # TOOL CHECK (everything EXCEPT quote_update!)
    # ==========================================
    tool_context = ""
    if USE_TOOLS:
        try:
            ...
            if tool_name and tool_info and tool_name not in ["quote_update", "update_website_quote"]:
                log("ALTPERSONA", f"🧰 Tool detected: {tool_name}", Fore.MAGENTA)
...
"""
                    log("ALTPERSONA", f"✅ Tool succeeded: {tool_name}", Fore.GREEN)
    ...
    try:
        ...
        res, used_model, _ = None, None, None
        for attempt in range(3):
            try:
                ...
                if attempt < 2:
                    log("ALTPERSONA", f"⏳ Rate limit — waiting {wait}s (attempt {attempt+1}/3)...", Fore.YELLOW)
        ...
        if altpersona_journal and res:
            try:
                ...
                # Diary logging is non-critical; never fail the response over it.
                pass
