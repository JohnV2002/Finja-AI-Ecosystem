/*
  YourAI Debug Frontend
  ====================
  Frontend module for the YourAI dashboard.

  Main Responsibilities:
  - Render debug events, logs, and developer controls.
  - Filter and format runtime events for inspection.
  - Provide quick debug commands for admin workflows.

  Side Effects:
  - Reads debug data from backend endpoints and WebSocket events.
  - Updates debug DOM panels and may trigger admin commands.
*/
const Debug = (() => {
  ...
  // ─── Public: clear ───────────────────────────────────────────
  function clear() {
    if (feedEl) feedEl.innerHTML = '<div class="debug-empty"><p>🔍 Waiting for pipeline events...</p></div>';
    eventCount = 0;
    _loadedCount = 0;
