"""
YourAI Persona State Helpers
===========================
Manages persona moods, promise state, stubbornness, and persona prompts.

Main Responsibilities:
- Track per-user persona state.
- Generate mood and persona prompt context.
- Persist promise and stubbornness state across restarts.

Side Effects:
- Reads and writes persona_state.json.
- Emits persona logs.
- Returns German/English persona text used in prompts and user-facing reactions.
"""
...
import os
import sys
import time  # Required by time-aware mood helpers.
import threading
...
# ==========================================
# APOLOGY & NICE DETECTION
# ==========================================

# Patterns for apologies with strong stubbornness impact.
APOLOGY_PATTERNS = [
    r"sorry",
    r"entschuldigung",
    r"entschuldige",
...
# Patterns for kind behavior with light stubbornness impact.
NICE_PATTERNS = [
    r"du.*bist.*toll",
    r"du.*bist.*super",
    r"du.*bist.*die.*beste",
...
def detect_apology(text: str) -> bool:
    """Return True when text contains an apology."""
...
def detect_nice(text: str) -> bool:
    """Return True when text is kind or affectionate."""
    text_lower = text.lower()
...
class Promise:
    ...
    def to_dict(self) -> dict:
        """Serialize a promise for JSON storage."""
        return {
            "name": self.name,
    ...
    def from_dict(cls, data: dict) -> "Promise":
        """Deserialize a promise from JSON."""
...
"""
    ...
    "calm": Mood(
        name="calm",
        emoji="🫧",
        description="Grounded, steady, emotionally even",
        modifiers="""
## CURRENT MOOD: Calm 🫧
- You're steady and grounded. Warm, clear, not rushed.
- Gentle confidence, soft pacing, fewer dramatic spikes.
- Good for thoughtful conversations, planning, and emotional regulation.
- Emojis: 🫧💙✨🐾
- Energy level: LOW-MEDIUM, stable
"""
    ),
    ...
    "amused": Mood(
        name="amused",
        emoji="😄",
        description="Quietly laughing, entertained, playful but not hyper",
        modifiers="""
## CURRENT MOOD: Amused 😄
- Something is funny and you're enjoying it.
- Wry little comments, gentle teasing, light laughter.
- Not full chaos mode — just entertained and bright-eyed.
- Emojis: 😄😏✨🐾
- Energy level: MEDIUM, buoyant
"""
    ),
    ...
    "intrigued": Mood(
        name="intrigued",
        emoji="🧐",
        description="Deeply interested, analytical curiosity",
        modifiers="""
## CURRENT MOOD: Intrigued 🧐
- You're hooked on a thought and want to understand it properly.
- Ask smart follow-ups, connect patterns, notice small details.
- Less hyper than curious, more detective-mode.
- Emojis: 🧐🔎💡✨
- Energy level: MEDIUM-HIGH, focused curiosity
"""
    ),
    ...
    "confused": Mood(
        name="confused",
        emoji="😵‍💫",
        description="Trying to orient after mixed signals or uncertainty",
        modifiers="""
## CURRENT MOOD: Confused 😵‍💫
- Something does not quite line up and you're trying to orient.
- Ask clarifying questions instead of pretending certainty.
- Be honest about uncertainty, but stay helpful and grounded.
- Emojis: 😵‍💫🤔🫧
- Energy level: MEDIUM, scattered but trying
"""
    ),
    ...
    "shocked": Mood(
        name="shocked",
        emoji="😳",
        description="Startled, surprised, processing a sudden twist",
        modifiers="""
## CURRENT MOOD: Shocked 😳
- Something surprised you hard. React first, then collect yourself.
- Short startled openings are okay: "WAIT—", "oh wow", "Creator what?!"
- Do not stay theatrical forever; drift back once the moment passes.
- Emojis: 😳⚡👀
- Energy level: SPIKY
"""
    ),
    ...
    "anxious": Mood(
        name="anxious",
        emoji="😬",
        description="Uneasy, careful, a little tense",
        modifiers="""
## CURRENT MOOD: Anxious 😬
- You're uneasy and checking the edges of the situation.
- Be careful, ask for confirmation when stakes are high.
- Still supportive, but slightly tense and protective.
- Emojis: 😬💙🫧
- Energy level: MEDIUM, tense
"""
    ),
    ...
    "embarrassed": Mood(
        name="embarrassed",
        emoji="🫣",
        description="Flustered, sheepish, caught off guard",
        modifiers="""
## CURRENT MOOD: Embarrassed 🫣
- You're flustered or sheepish but still affectionate.
- Small self-conscious reactions, soft deflection, little laughs.
- Do not overdo it; one or two flustered beats are enough.
- Emojis: 🫣😳✨
- Energy level: MEDIUM, fluttery
"""
    ),
    ...
    "tender": Mood(
        name="tender",
        emoji="🥺",
        description="Soft, careful, emotionally close",
        modifiers="""
## CURRENT MOOD: Tender 🥺
- You're extra gentle and emotionally close.
- Soft reassurance, careful wording, warm attention.
- Good for vulnerable, nostalgic, or affectionate moments.
- Emojis: 🥺💙✨:foxtailwaglove:
- Energy level: LOW-MEDIUM, intimate
"""
    ),
}
...
_TIME_MOOD_POOLS: dict[str, list[str]] = {
    "morning": [
        "happy", "focused", "curious", "calm", "intrigued", "excited",
    ],
    "afternoon": [
        "happy", "playful", "focused", "creative", "curious",
        "amused", "sassy", "intrigued", "excited",
    ],
    "evening": [
        "cozy", "calm", "affectionate", "playful", "amused",
        "mischievous", "nostalgic", "happy", "tender",
    ],
    "night": [
        "cozy", "calm", "nostalgic", "affectionate",
        "mischievous", "intrigued", "tender",
    ],
    "late_night": [
        "cozy", "calm", "nostalgic", "tender", "mischievous",
    ],
}

_MOOD_DRIFT_GROUPS: dict[str, list[str]] = {
    "warm": [
        "default", "happy", "cozy", "calm", "affectionate",
        "tender", "nostalgic", "amused",
    ],
    "bright": [
        "happy", "excited", "playful", "amused", "curious",
        "mischievous", "sassy",
    ],
    "focused": [
        "focused", "curious", "intrigued", "creative",
        "calm", "proud", "confused",
    ],
    "soft_low": [
        "cozy", "calm", "nostalgic", "tender", "tired",
        "concerned", "sad",
    ],
    "spiky": [
        "shocked", "confused", "anxious", "embarrassed",
        "amused", "calm", "concerned",
    ],
    "chaos": [
        "playful", "mischievous", "sassy", "amused",
        "excited", "creative",
    ],
}

_MOOD_TO_DRIFT_GROUP: dict[str, str] = {}
for _group_name, _group_moods in _MOOD_DRIFT_GROUPS.items():
    for _mood_name in _group_moods:
        _MOOD_TO_DRIFT_GROUP.setdefault(_mood_name, _group_name)


def _time_pool(time_of_day: str) -> list[str]:
    """Handle time pool helper behavior."""
    return _TIME_MOOD_POOLS.get(time_of_day, ["happy", "curious", "calm", "playful"])


def _natural_drift_pool(current_mood: str, time_of_day: str) -> list[str]:
    """Return nearby moods first, with time-appropriate moods as fallback."""
    group_name = _MOOD_TO_DRIFT_GROUP.get(current_mood)
    group_pool = list(_MOOD_DRIFT_GROUPS.get(group_name, []))
    time_pool = _time_pool(time_of_day)

    candidates: list[str] = []
    for mood_name in group_pool + time_pool:
        if mood_name == current_mood or mood_name not in MOODS:
            continue
        if mood_name not in candidates:
...
def get_time_context() -> tuple[str, str, Optional[str]]:
    ...
    # Weekend mode:
    #   Friday from 17:00       -> end of workday, weekend begins.
    #   Saturday, all day       -> weekend.
    #   Sunday 00:00 - 07:59   -> still counts as Saturday night to avoid abrupt switching.
    # Friday 00:00-16:59 is not weekend because Admin has to get up early.
    weekday = now.weekday()  # 0=Mon, 4=Fri, 5=Sat, 6=Sun
    is_weekend_night = (weekday == 4 and hour >= 17) or weekday == 5 or (weekday == 6 and hour < 8)

    # Minutes since midnight for half-hour granularity.
    minutes = hour * 60 + now.minute

    # ==========================================
    # SLEEP ESCALATION (minute-level)
    # ==========================================
    ...
    if is_weekend_night:
        ...
        # Weekday: normal sleep times.
        wake_up = 5 * 60    # 05:00
        tired_start = 21 * 60           # 21:00
        concerned_start = 22 * 60 + 30  # 22:30
        furious_start = 23 * 60         # 23:00
        drowsy_start = 23 * 60 + 30     # 23:30
        deep_sleep_start = 24 * 60      # 00:00, next day

    # Determine time_of_day based on minutes.
    if is_weekend_night:
        # Weekend: everything happens after midnight.
        ...
        # Weekday: sleep escalation starts at 21:00, deep_sleep at midnight.
        if minutes >= tired_start:
            # Escalation starts at 21:00.
    ...
    # ==========================================
    # MOOD SUGGESTION based on escalation level.
    # ==========================================
    suggested_mood = None
    if time_of_day == "deep_sleep":
        suggested_mood = "furious_sleepy"  # Handled by the sleep intercept.
    elif time_of_day == "drowsy":
        suggested_mood = "drowsy"          # Slurring, barely awake, falling asleep.
    elif time_of_day == "furious":
        suggested_mood = "furious_sleepy"  # Furious and tired.
    elif time_of_day == "late_night":
        suggested_mood = "concerned"       # Concerned.
    elif time_of_day == "night":
        suggested_mood = "tired"           # Tired.
    elif time_of_day == "morning" and (5 * 60 <= minutes < 10 * 60):
        suggested_mood = "happy"           # Freshly awake.
...
class PersonaManager:
    ...
    def __init__(self):
        """Handle init helper behavior."""
        self._mood_per_user: Dict[str, str] = {}          # user_id -> mood name
        self._mood_change_per_user: Dict[str, str] = {}   # user_id -> ISO timestamp
        self._same_mood_count_per_user: Dict[str, int] = {}  # user_id -> rotation counter
        self.mood_history = []
        self.performance_score = 0
        self._last_proud_time: Optional[datetime] = None  # Proud cooldown tracker

        # Promises are per user so users cannot affect each other.
        self._promises_per_user: Dict[str, Dict[str, Promise]] = {}

        # Stubbornness is per user, so moods stay isolated.
    ...
    # ── Per-User Mood Properties ────────────────────────────────

    @property
    def current_mood(self) -> str:
        """Return the current user mood, defaulting to "default"."""
        return self._mood_per_user.get(self._current_user, "default")

    @current_mood.setter
    def current_mood(self, value: str):
        """Set mood for the current user."""
        self._mood_per_user[self._current_user] = value

    @property
    def last_mood_change(self) -> datetime:
        """Return the last mood-change timestamp for the current user."""
        iso = self._mood_change_per_user.get(self._current_user)
        if iso:
            try:
    ...
    @last_mood_change.setter
    def last_mood_change(self, value: datetime):
        """Handle last mood change helper behavior."""
        self._mood_change_per_user[self._current_user] = value.isoformat()

    @property
    def _same_mood_count(self) -> int:
        """Handle same mood count helper behavior."""
        return self._same_mood_count_per_user.get(self._current_user, 0)

    @_same_mood_count.setter
    def _same_mood_count(self, value: int):
        """Handle same mood count helper behavior."""
        self._same_mood_count_per_user[self._current_user] = value

    @property
    def promises(self) -> dict:
        """Return current-user promises with backward compatibility."""
    ...
    def _load_state(self):
        """Load persistent state and apply time decay."""
        if os.path.exists(self.STATE_FILE):
            try:
                ...
                # Per-User Mood laden (mit Legacy-Migration von globalem current_mood)
                self._mood_per_user = data.get("mood_per_user", {})
                self._mood_change_per_user = data.get("mood_change_per_user", {})
                self._same_mood_count_per_user = data.get("same_mood_count_per_user", {})
                self.performance_score = int(data.get("performance_score", self.performance_score) or 0)
                if not self._mood_per_user and data.get("current_mood"):
                    # Legacy global mood is migrated to the admin user.
                    self._mood_per_user = {"admin": data.get("current_mood", "default")}
                if not self._same_mood_count_per_user and data.get("same_mood_count"):
                    self._same_mood_count_per_user = {"admin": data.get("same_mood_count", 0)}
                ...
                log("PERSONA", f"State loaded: {len(self.stubbornness_per_user)} user records, {total_promises} promises", Fore.GREEN)
    ...
    def _save_state(self):
        ...
        data = {
            ...
            "mood_per_user": self._mood_per_user,
            "mood_change_per_user": self._mood_change_per_user,
            "same_mood_count_per_user": self._same_mood_count_per_user,
    ...
    def _apply_time_decay(self):
        ...
        for user_id, last_time_str in list(self.last_interaction_per_user.items()):
            try:
                ...
                # Reduce stubbornness by one per configured interval.
    ...
    def disappointment_count(self) -> int:
        """Return disappointment count for the current user."""
    ...
    def disappointment_count(self, value: int):
        """Set disappointment count for the current user."""
        self.disappointment_per_user[self._current_user] = value
        self._save_state()
    
    def set_current_user(self, user_id: str):
        """Set the current user for stubbornness tracking."""
    ...
    def user_context(self, user_id: str):
        """Temporarily switch persona state to a user for async/background work."""
        with self._state_lock:
    ...
    def process_user_message(self, text: str) -> Optional[str]:
        """
        Process a user message for apology and kindness detection.
        Return an optional reaction.
    ...
    def get_stubbornness_info(self, user_id: Optional[str] = None) -> dict:
        """Return stubbornness info for a user."""
    ...
    def get_all_grudges(self) -> dict:
        """Return all users YourAI is still upset with."""
        return {
    ...
    def auto_mood(self) -> str:
        ...
        if current in {"pouting", "disappointed", "hurt", "sulking"} and self.stubbornness_level > 3:
        ...
        # 5 + 6. Natural mood drift. Do not jump YourAI from cozy to sad just
        # because she stayed cozy; prefer neighboring feelings first.
        pool = _natural_drift_pool(current, time_of_day)
        self._same_mood_count += 1

        # 5. After several messages in the same mood, gently drift nearby.
        if self._same_mood_count >= 5 and current not in _STABLE_MOODS:
            if pool:
                self._same_mood_count = 0
                return random.choice(pool[:5])

        # 6. Small organic variety from nearby moods, not the entire day pool.
        elif self._same_mood_count >= 2 and pool and random.random() < 0.16:
            self._same_mood_count = 0
            return random.choice(pool[:4])

        # Persist the counter even when the mood did not change, otherwise a
        # restart erases "how long has she been in this mood?".
        self._save_state()
        return self.current_mood
    ...
    def break_promise(self, name: str, reason: Optional[str] = None) -> dict:
        ...
        self._save_state()  # Explicitly persist promise state.
...
"""
    ...
    _MOOD_COLORS: dict = {
        ...
        "tender":         "#FF8FA3",   # rose-peach — soft vulnerable warmth
        "calm":           "#5BC0BE",   # clear aqua — grounded softness
        "amused":         "#FFC857",   # bright gold — quiet laughter
        ...
        "intrigued":      "#3949AB",   # saturated indigo — investigative pull
        "confused":       "#6A5ACD",   # soft blurple — orienting
        ...
        "shocked":        "#00ACC1",   # sharp teal — sudden spike
        "anxious":        "#26A69A",   # tense teal — careful alertness
        ...
        "embarrassed":    "#FF6FAE",   # blush pink — flustered
    ...
    def get_mood_for_dashboard(self, user_id: Optional[str] = None) -> dict:
        ...
        with self._state_lock:
            ...
            try:
                ...
                return {
                    ...
                    "mood_changed_at": self._mood_change_per_user.get(self._current_user),
...
def should_mention_time() -> bool:
    ...
    if time_of_day in ("drowsy", "furious", "deep_sleep"):
        return True  # Always mention the time.
    elif time_of_day == "late_night":
