"""
YourAI Semantic Prompt Router
============================
Semantic classifier that selects which optional prompt sections should be injected.

Main Responsibilities:
- Classify user messages into route labels for relevant tools or domains.
- Cache route embeddings and initialize prompt routing lazily.
- Reduce prompt size by avoiding unrelated tool sections.

Side Effects:
- Reads and writes a local embedding cache.
- Calls embedding providers through OpenRouter during initialization or cache refresh.
"""
...
_ROUTE_UTTERANCES: dict[str, list[str]] = {
    ...
    "file": [
        ...
        "kannst du Kapitel 2 lesen", "kannst du Chapter 2 lesen",
        "lies Kapitel 2 aus meinem Buch", "read Book 2 chapter 2",
        "zeige mir Kapitel 2", "was passiert in Kapitel 2",
        "lies die hochgeladene Datei", "read the attached document",
    ],
    "image": [
...
def _cosine_sim(a: list[float], b: list[float]) -> float:
    """
    Executes the _cosine_sim helper logic.
...
def _load_cache() -> dict[str, list[float]] | None:
    """
    Executes the _load_cache helper logic.
...
def _save_cache(centroids: dict[str, list[float]]):
    """
    Executes the _save_cache helper logic.
...
async def _ensure_initialized():
    """
    Executes the _ensure_initialized helper logic.
