...
===========================
An esoteric programming language where Python code looks like
absolute nonsense but runs natively.
...
# Rule: everything sounds like a crazy alien gremlin
...
# Sorted by length (longest first) to avoid partial overlaps
...
def glorpify(python_code: str) -> str:
    """Translate Python code into Glorpo code."""
    return _translate(python_code, _GLORPO_SORTED)


def deglorpify(glorpo_code: str) -> str:
    """Translate Glorpo code back into Python code."""
...
def run_glp(filepath: str, glorpo_globals: dict = None) -> None:
    ...
    3. exec() with its own namespace
    4. On error: attach the Glorpo source line to the traceback
    """
    ...
    # __glorpmain__ inside strings is not deglorpified (correct),
    # so check whether the code uses __glorpmain__ in a string comparison
...
def run_glorpo_string(glorpo_code: str) -> str:
    """
    Execute Glorpo code given as a string and capture stdout.
    Returns the output as a string.
    On error: attach the Glorpo source line to the traceback.
    """
    import io
...
def _cli_main():
    """Entry point for the `glorpo` CLI command."""
    import argparse
