...
=========================
Tools für YourAI's Website (your-domain.example.com)
...
# Kategorien mit jeweils eigenem Vibe
QUOTE_CATEGORIES = [
    {
...
def generate_quote_prompt() -> str:
    """Baut einen randomisierten Quote-Prompt mit zufälliger Kategorie."""
    # Zufällige Kategorie wählen
    category = random.choice(QUOTE_CATEGORIES)
    
    # Zufällig ein Beispiel wählen (nicht beide, sonst kopiert das Model)
    example = random.choice(category["examples"])
    
    # Sprache zufällig wählen
...
# Legacy alias (generiert EIN Mal beim Import - für Kompatibilität)
...
def update_quote(context: Dict[str, Any], debug: Any = None) -> Dict[str, Any]:
    """
    Generiert einen neuen Spruch und updated die Website.
    
    Args:
        context: Dict mit "question", "user_name", etc.
                 Optional: "quote_text" wenn schon generiert
                 Optional: "mood" für den Spruch
        debug: Dashboard debug client
        
    Returns:
        {"success": bool, "result": str, "error": str|None, "quote": str}
    """
    if debug is None:
        class DummyDebug:
    ...
    # Check ob Quote schon generiert wurde
    quote_text = context.get("quote_text")
    mood = context.get("mood", "playful")
    
    if not quote_text:
        # Quote muss noch generiert werden
        # Das passiert in yourai_node BEVOR dieses Tool aufgerufen wird
...
def get_current_quote(debug: Any = None) -> Dict[str, Any]:
    """
    Holt den aktuellen Spruch von der Website.
    
    Returns:
        {"success": bool, "quote": dict|None, "error": str|None}
    """
