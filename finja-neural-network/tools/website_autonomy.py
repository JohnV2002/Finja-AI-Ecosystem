...
=======================================
YourAI kann ihre eigene Website (yourai.html) selbstständig anpassen.
...
def _validate_html(new_html: str, old_html: str) -> tuple[bool, list[str]]:
    ...
    old_emoji_count = _count_emoji_and_unicode(old_html)
    new_emoji_count = _count_emoji_and_unicode(new_html)
...
def _fetch_file(local_path: Optional[str], remote_url: str, max_chars: int, label: str) -> str:
    if local_path and os.path.exists(local_path):
        try:
            ...
            log("WEBSITE_AUTO", f"   {label}: {len(content)} chars (lokal: {local_path})", Fore.CYAN)
            return content
...
def _count_emoji_and_unicode(text: str) -> int:
...
REDESIGN_COOLDOWN_SECONDS = 600   # 10 Minuten zwischen Redesigns
REDESIGN_MAX_PER_DAY = 5          # Max 5 Redesigns pro Tag
HTML_AUTO_DOWNGRADE_THRESHOLD = 20000  # >20k HTML chars → both wird zu css

# ==========================================
# MAIN LOGIC
# ==========================================
def _run_website_update(debug: Any, yourai_hint: str = ""):
    """Hauptlogik — wird im Background Thread ausgeführt."""
    try:
        _run_website_update_inner(debug, yourai_hint)
    except Exception as e:
        # NIEMALS den Thread still sterben lassen
...
def _run_website_update_inner(debug: Any, yourai_hint: str = ""):
    ...
    try:
        ...
        # Wenn YourAI einen konkreten Wunsch hatte, wird er VORNE eingebaut (vor den Regeln!)
        hint_block = ""
        if yourai_hint:
        ...
        if config.USE_OPENROUTER:
            yourai_response, _ = call_openrouter(
                ...
                model="google/gemma-4-26b-a4b-it",  # Kein Thinking, folgt Formaten brav, ~22% günstiger als Nemotron
                temperature=0.9,
                max_tokens=2500
            )
        else:
    ...
    # SAFETY: Auto-Upgrade "both" → "all" wenn JS-Keywords in den Changes stecken
    # (YourAI wählt manchmal "both" obwohl JS gebraucht wird — stille Fehler verhindern!)
    if file_target == "both":
        _JS_KEYWORDS = [
    ...
    # SAFETY: Auto-Downgrade "both"/"html"/"all" → "css" wenn HTML zu groß ist
...
3. Output ONLY the JS, no explanations, no markdown code blocks\nOUTPUT THE COMPLETE JS:"""
            ...
            code_prompt = PROMPT_CODE_CHANGES.format(current_html=current_html, current_css="/* CSS not relevant for this change */", changes=changes_text)
            code_response, _ = call_openrouter(
            ...
            new_html = _extract_between(code_response, "===HTML_START===", "===HTML_END===")
            if not new_html:
            ...
            if not new_html:
                ...
                if debug: debug.error("website_auto", f"❌ {err.short()}")
                return
            # Truncation guard: wenn Output < 50% vom Input → Model hat abgeschnitten
...
OUTPUT THE COMPLETE JS:"""
    ...
    try:
        ...
        if resp.status_code == 200 and resp.json().get("success"):
            ...
            # Lokale Backup-Kopien speichern
        ...
        # Deploy fehlgeschlagen (HTTP != 200 oder success != true)
...
def maybe_trigger_website_update(debug: Any = None, force: bool = False, yourai_hint: str = "") -> bool:
    ...
    # SAFETY 1: Concurrent Lock — nur ein Redesign gleichzeitig
    ...
    try:
        # SAFETY 2: Cooldown — min 10 Minuten zwischen Redesigns
        ...
        # SAFETY 3: Daily Limit — max N Redesigns pro Tag
