...
===============================================
HTTP-Client für den DuckDuckGo/Tor Web Crawler Docker Container.
...
def web_search(query: str, count: Optional[int] = None, debug: Any = None) -> Dict[str, Any]:
    """
    Sucht im Web via Docker Crawler.

    Args:
        query: Suchbegriff
        count: Anzahl gewünschter Ergebnisse (default: WEB_CRAWLER_MAX_RESULTS)
        debug: Dashboard debug client

    Returns:
        {"success": bool, "results": [...], "message": str, "query": str}
    """
    ...
    try:
        ...
        # RERANKER: Ergebnisse nach Relevanz zur Query sortieren (wenn verfügbar)
        try:
            ...
            pass  # Graceful fallback — Originalreihenfolge bleibt erhalten

        # Lesbaren Text für YourAI bauen
        result_lines = []
        ...
        err = YourAIToolExecutionError("Fehler bei der Websuche", tool_name="web_search", cause=e)
        log_exception("WEB", err)
