...
----------------------------------------------------------------------
 ...
 ---------------------------------------------------------------------
  ...
  • 📂 Added .json/.js extension check for config file to prevent misuse
  • ✅ Fixed filename suffix handling in atomic writes via .with_suffix()
...
def write_atomic(path: str, text: str) -> None:
    """Atomar in eine Datei schreiben (mit Tempfile + Replace)."""
    try:
        ...
        while True:
            try:
                ...
                try:
                    ...
                    write_atomic(os.path.join(out_dir, "obs_react.txt"), "Bye 👋❤")
                    # Optional: Genres leeren und/oder NowPlaying neutralisieren
                    write_atomic(os.path.join(out_dir, "obs_genres.txt"), "")
