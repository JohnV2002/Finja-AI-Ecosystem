...
======================================================================
  ...
  Version: 1.4.1 (TruckersFM Modul)
...
----------------------------------------------------------------------
 ...
 ---------------------------------------------------------------------
  ...
  • UTC-Timestamps + Sync-Guard gegen halbgeschriebene nowplaying.txt
  • RAM-basierte Knowledge Base + SHA-256 Index-Cache
  • Result-LRU-Cache (inkl. Negativ-Caching)
  • Debug-Hook (kb_probe.py)
  • Specials (Rickroll, Crab Rave etc.) + External reactions.json
  • Context-/Game-Bias via contexts.json + game_state.txt
  • Unknown/Explore-Policy für neue Songs
  • Listening-Phase: erst „Listening…“ dann finale Reaktion
  • Artist-Preferences: score_bias + probabilistische Flips
  • Fuzzy-Matching (robuster gegen Schreibweisen)
  • Special-Version-Tags (nightcore/speed up/tiktok/radio edit)
  • Memory-System: bewertet Songs pro Kontext (mit Decay + Tuning)
  • Variants: „immer noch …“, „… aber in {game} besser“, „… passt zu {game}“

----------------------------------------------------------------------
 Neu in v1.4.1:
 ---------------------------------------------------------------------
  • Artist-Preferences (leichtgewichtig, entkoppelt von Memory):
      reactions.artist_preferences = {
          "artist": { "score_bias": 2.0, "flip": { "dislike": 0.40 } }
      }
     → addiert +2.0 Punkte für Künstler-Matches,
       lässt aber mit 40% Wahrscheinlichkeit zu, dass das Ergebnis trotzdem „dislike“ wird.
     ⚠️ Hinweis: kein Hard-Override – nur Bias + probabilistischer Flip
...
import json, os, re, time, hashlib, collections, pickle, subprocess, sys, uuid, random
