...
----------------------------------------------------------------------
 Features:
 ---------------------------------------------------------------------
  • Verbindet sich mit einem laufenden Chrome-Browser über das Chrome DevTools Protocol (CDP).
  • Führt JavaScript auf der 89.0 RTL-Webseite aus, um den aktuellen Song auszulesen.
  • Nutzt eine "Stabilizer"-Klasse, um nur stabile Songtitel zu akzeptieren und Flackern zu vermeiden.
  • Schreibt den erkannten Song atomar in die `nowplaying.txt`.
  • Priorisiert automatisch den richtigen Browser-Tab (z.B. Radioplayer vor Hauptseite).
...
def main():
    ...
    ap.add_argument("--out", default="Nowplaying/nowplaying.txt")
