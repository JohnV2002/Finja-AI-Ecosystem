...
----------------------------------------------------------------------
 Updates 4.4.5:
 ---------------------------------------------------------------------
  + **TTS Cache Hash Normalization:** Text is now normalized (leftover
    markdown syntax stripped, whitespace collapsed) before hashing in
    `/upload_tts_cache` and `/get_tts_audio`. Trivial formatting/spacing
    differences in otherwise identical text no longer cause avoidable
    cache misses. Only affects the cache key — the stored/served audio
    is untouched.

 Updates 4.4.4:
 ---------------------------------------------------------------------
  + **Provider-Scoped TTS Cache:** `/upload_tts_cache` and `/get_tts_audio`
    now require a `provider` field (e.g. "chatterbox", "elevenlabs"). It is
    folded into the cache key, so switching TTS provider/tier no longer
    serves stale audio generated by a different engine for the same text.
    Old (pre-4.4.4) cache entries are keyed without a provider and will
    simply miss going forward — they are not migrated or deleted.
...
MEMORY_VECTOR_SEARCH_ENABLED = os.getenv("MEMORY_VECTOR_SEARCH_ENABLED", "1").lower() not in ("0", "false", "no", "off")
MEMORY_VECTOR_MAX_CANDIDATES = int(os.getenv("MEMORY_VECTOR_MAX_CANDIDATES", "5000"))
MEMORY_VECTOR_DIM = int(os.getenv("MEMORY_VECTOR_DIM", "512"))
MEMORY_IMPORTANCE_WEIGHT = float(os.getenv("MEMORY_IMPORTANCE_WEIGHT", "0.2"))
MEMORY_CLUSTER_CAP = int(os.getenv("MEMORY_CLUSTER_CAP", "3"))

ENCRYPTION_MAGIC = b"FINJAENC1\n"
NONCE_SIZE = 12
...
def _vector_cache_key(text: str) -> str:
    return hashlib.sha1(f"{MEMORY_VECTOR_DIM}\0{text}".encode("utf-8")).hexdigest()


def _tokenize_for_vector(text: str) -> List[str]:
    return re.findall(r"[\wÄÖÜäöüß]{2,}", (text or "").casefold())


def _local_hash_vector(text: str) -> List[float]:
    """Build a deterministic local bag-of-words vector without external APIs."""
    key = _vector_cache_key(text)
    with vector_cache_lock:
        cached = vector_cache.get(key)
        if cached is not None:
            return cached

    dim = max(64, MEMORY_VECTOR_DIM)
    vec = [0.0] * dim
...
def _rank_memories_local_vector(query: str, memories: List["MemoryItem"], limit: int) -> Optional[List["MemoryItem"]]:
    """Rank all user memories locally by hashed-vector cosine and hygiene metadata."""
    if not (MEMORY_VECTOR_SEARCH_ENABLED and query):
        return None

    candidates = [m for m in memories if getattr(m, "text", "")]
    max_candidates = max(limit, MEMORY_VECTOR_MAX_CANDIDATES)
    candidates = candidates[-max_candidates:]
    if not candidates:
        return []

    query_vec = _local_hash_vector(query)
    weight = max(0.0, min(1.0, MEMORY_IMPORTANCE_WEIGHT))
    scored = []
    for mem in candidates:
        meta = mem.meta if isinstance(mem.meta, dict) else {}
        sim = _cosine(query_vec, _local_hash_vector(mem.text))
        importance = _safe_float(meta.get("importance_score"), 0.5)
        combined = (1.0 - weight) * sim + weight * importance
        scored.append((combined, sim, mem, meta.get("semantic_cluster_id")))

    scored.sort(key=lambda row: row[0], reverse=True)
    selected: List[MemoryItem] = []
    cluster_counts: Dict[Any, int] = {}
    cluster_cap = max(1, MEMORY_CLUSTER_CAP)
    for _, _, mem, cluster_id in scored:
...
def encryption_enabled() -> bool:
    return _encryption_key_bytes() is not None
...
def encrypt_bytes(data: bytes) -> bytes:
    ...
    if key is None:
        return data
    if AESGCM is None:
        raise RuntimeError("cryptography is required for FINJA_DATA_ENCRYPTION_KEY")
...
def decrypt_bytes(data: bytes) -> bytes:
    ...
    key = _encryption_key_bytes()
    if key is None:
        raise RuntimeError("Encrypted memory store requires FINJA_DATA_ENCRYPTION_KEY")
    if AESGCM is None:
        raise RuntimeError("cryptography is required for encrypted memory stores")
    payload = data[len(ENCRYPTION_MAGIC):]
    if len(payload) <= NONCE_SIZE:
        raise RuntimeError("Encrypted memory store is truncated")
...
def save_to_disk(user_id):
    ...
    if user_id in user_memories:
        try:
            ...
            write_json_file_atomic(filepath, [m.model_dump() for m in user_memories[user_id]])
...
def load_from_disk(user_id):
    ...
    filepath = memory_file(user_id)
    try:
        items = read_json_file(filepath, default=[])
...
def get_memories(request: Request, user_id: Optional[str] = None, query: Optional[str] = None, limit: int = 50):
    ...
    cache_hit = uid in user_memories
    limit = max(1, min(int(limit or 50), MAX_RAM_MEMORIES))
    ...
    if query:
        try:
            ranked = _rank_memories_local_vector(query, memories, limit)
            if ranked is not None:
                filtered = ranked
                search_mode = "local_vector"
            else:
            ...
            filtered = memories
            search_mode = "fallback_latest"

    # Return the latest 'limit' matching memories
    result = filtered[:limit] if query and search_mode == "local_vector" else filtered[-limit:]
    emit_metric(
        "memory_search",
        "Memory search",
        duration_ms=_duration_ms(started_at),
        ...
        cache_hit=cache_hit,
        user_id=uid,
        details=f"query={'yes' if query else 'no'}; mode={search_mode}; limit={limit}; cache_hit={'yes' if cache_hit else 'no'}",
    )
    return result
...
# -------------------------
# Hygiene / Phase 2 endpoints (usage tracking, quarantine, merge support)
# -------------------------

class TouchAction(BaseModel):
    user_id: str
    texts: List[str]
...
class DeleteTextsAction(BaseModel):
    user_id: str
    texts: List[str]

@app.post("/touch_memories", responses={401: {"description": "Unauthorized"}})
def touch_memories(request: Request, data: Annotated[TouchAction, Body(...)]):
    """Mark memories as used: bump meta.use_count and set meta.last_used_at.

    Lazily initializes meta for legacy memories that were stored without it.
    """
    started_at = time.time()
    auth_check(request)
    uid = data.user_id
    ...
    wanted = {t.strip() for t in data.texts if t and t.strip()}
    touched = 0
    now = time.time()
    for m in user_memories.get(uid, []):
        if m.text.strip() in wanted:
            if not isinstance(m.meta, dict):
                m.meta = {}
            m.meta["use_count"] = int(m.meta.get("use_count") or 0) + 1
            m.meta["last_used_at"] = now
            touched += 1
    if touched:
        save_to_disk(uid)
    emit_metric(
        "memory_touch", "Memory touch",
        duration_ms=_duration_ms(started_at), result_count=touched,
        candidate_count=len(wanted), user_id=uid,
        details=f"touched={touched}/{len(wanted)}",
    )
    return {"status": "ok", "touched": touched, "requested": len(wanted)}

@app.post("/update_memories", responses={401: {"description": "Unauthorized"}})
def update_memories(request: Request, data: Annotated[UpdateAction, Body(...)]):
    """Apply per-memory bank changes and meta patches (hygiene: scores, cluster ids, quarantine)."""
    started_at = time.time()
    auth_check(request)
    uid = data.user_id
    if uid not in user_memories: load_from_disk(uid)
    ...
    by_text = {u.text.strip(): u for u in data.updates if u.text and u.text.strip()}
    updated = 0
    for m in user_memories.get(uid, []):
        u = by_text.get(m.text.strip())
        if not u:
            continue
        ...
        if u.meta_patch:
            if not isinstance(m.meta, dict):
                m.meta = {}
            m.meta.update(u.meta_patch)
        updated += 1
    if updated:
        save_to_disk(uid)
    emit_metric(
        "memory_update", "Memory hygiene update",
        duration_ms=_duration_ms(started_at), result_count=updated,
        candidate_count=len(by_text), user_id=uid,
        details=f"updated={updated}/{len(by_text)}",
    )
    return {"status": "ok", "updated": updated, "requested": len(by_text)}

@app.post("/delete_memories", responses={401: {"description": "Unauthorized"}})
def delete_memories(request: Request, data: Annotated[DeleteTextsAction, Body(...)]):
    """Delete specific memories by exact text (hygiene merge: duplicates fold into one survivor)."""
    started_at = time.time()
    auth_check(request)
    uid = data.user_id
    if uid not in user_memories: load_from_disk(uid)
    ...
    wanted = {t.strip() for t in data.texts if t and t.strip()}
    before = len(user_memories.get(uid, []))
    user_memories[uid] = [m for m in user_memories.get(uid, []) if m.text.strip() not in wanted]
    deleted = before - len(user_memories[uid])
    if deleted:
        save_to_disk(uid)
    emit_metric(
        "memory_delete", "Memory hygiene delete",
        duration_ms=_duration_ms(started_at), result_count=deleted,
        candidate_count=len(wanted), user_id=uid,
        details=f"deleted={deleted}/{len(wanted)}",
    )
    return {"status": "ok", "deleted": deleted, "requested": len(wanted)}
...
def _normalize_for_tts_cache(text: str) -> str:
    """Collapses whitespace and strips leftover markdown syntax so texts that
    sound identical also hash identically, even if formatting/spacing differs."""
    normalized = _TTS_MARKDOWN_RE.sub('', text)
    normalized = _TTS_WHITESPACE_RE.sub(' ', normalized)
    return normalized.strip()

@app.post("/upload_tts_cache", responses={401: {"description": "Unauthorized"}, 500: {"description": "Server Error"}})
async def upload_tts_cache(request: Request, text: Annotated[str, Form(...)], provider: Annotated[str, Form(...)], file: Annotated[UploadFile, File(...)]):
    """Receives generated TTS audio and stores it in the cache, keyed by provider+text."""
    started_at = time.time()
    auth_check(request)
    
    # Calculate hash to determine filename, keep uploaded format (.mp3 or .wav)
    cache_key = f"{provider.strip().lower()}|{_normalize_for_tts_cache(text)}"
...
def get_tts_audio(request: Request, text: str, provider: str):
    """
    Checks if audio for the text (as generated by the given provider) exists.
    ...
    emit_metric(
        ...
        cache_hit=False,
        status="warning",
        details=f"miss; provider={provider}",
    )
